...
const generalFaqs: FAQ[] = [
  ...
  {
    id: "unlock-but-keep-staked",
    question: "Can I unlock but keep my NEAR staked during the 45 days?",
    answer: (
      <div className="space-y-4">
        <Text>
...
const InfoFAQ = () => {
  ...
  const isValidFaqId =
    faqId && [...generalFaqs, ...walletFaqs].some((faq) => faq.id === faqId);
  ...
  const handleToggle = useCallback(
    (value: string) => {
      if (value) {
        setOpenItem(value);
        const faq = [...generalFaqs, ...walletFaqs].find((f) => f.id === value);
        trackEvent({
        ...
        if (openItem) {
          const faq = [...generalFaqs, ...walletFaqs].find(
            (f) => f.id === openItem
          );
          trackEvent({
